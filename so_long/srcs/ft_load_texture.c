...
/*   Updated: 2021/07/16 18:59:28 by gejo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_player_texture(t_game *game)
{
	...
	game->tex_player.data = (int *)mlx_get_data_addr(
			game->tex_player.img, &game->tex_player.bpp,
			&game->tex_player.size_l, &game->tex_player.endian);
...
void	ft_collect_texture(t_game *game)
{
	...
	game->tex_collect.data = (int *)mlx_get_data_addr(
			game->tex_collect.img, &game->tex_collect.bpp,
			&game->tex_collect.size_l, &game->tex_collect.endian);
...
void	ft_exit_texture(t_game *game)
{
	...
	game->tex_exit.data = (int *)mlx_get_data_addr(
			game->tex_exit.img, &game->tex_exit.bpp,
			&game->tex_exit.size_l, &game->tex_exit.endian);
...
void	ft_load_texture(t_game *game)
{
	...
	game->tex_wall.data = (int *)mlx_get_data_addr(
			game->tex_wall.img, &game->tex_wall.bpp,
			&game->tex_wall.size_l, &game->tex_wall.endian);
	game->tex_tile.img = mlx_xpm_file_to_image(
			game->mlx_ptr, "./texture/tile.xpm",
			&game->tex_tile.width, &game->tex_tile.height);
	game->tex_tile.data = (int *)mlx_get_data_addr(
			game->tex_tile.img, &game->tex_tile.bpp,
			&game->tex_tile.size_l, &game->tex_tile.endian);
	game->tex_stone.img = mlx_xpm_file_to_image(
			game->mlx_ptr, "./texture/b_stone.xpm",
			&game->tex_stone.width, &game->tex_stone.height);
	game->tex_stone.data = (int *)mlx_get_data_addr(
			game->tex_stone.img, &game->tex_stone.bpp,
